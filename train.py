"""
python train.py --model-config model_configs/config.yaml --data-path data/Loaded_MP_bandgap-sphere-images_standardized --dest-path exps/train/single_sample/1000_eras_batch_size_512_random_noise_with_eval_001_data  --pin-memory --check-labels --batch-size 512 --shuffle
"""
...
def main():
    ...
    parser.add_argument("--train-portion", type=float, default=0.01, help="Portion of subset to be used for training.")
    ...
    # Train the model
    start = time.time()
    model.train(train_dataloader, val_dataloader, exp_name=args.dest_path, train_specific_step=args.train_specific_step, val_jump=1) #set val_jump to something else IF NEEDED
    end = time.time()
